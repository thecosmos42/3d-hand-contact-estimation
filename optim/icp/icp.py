...
# ! This codebase is adapted from the PyTorch3D library.
import warnings
...
def ICP(
    obj_contact_pcd: Union[torch.Tensor, "Pointclouds"],
    hum_contact_pcd: Union[torch.Tensor, "Pointclouds"],
    ...
    verbose: bool = False,
    obj_contact_normals: Optional[torch.Tensor] = None,
    hum_contact_normals: Optional[torch.Tensor] = None,
    min_scale: float = None,
    scale_penalty: float = 10.0,
) -> ICPSolution:
    """
    ...
    `d`-dimensional points `obj_contact_pcd` and `hum_contact_pcd`, such that:

    `s[i] obj_contact_pcd[i] R[i] + T[i] = hum_contact_pcd[NN[i]]`,

    for all batch indices `i` in the least squares sense. Here, hum_contact_pcd[NN[i]] stands
    for the indices of nearest neighbors from `hum_contact_pcd` to each point in `obj_contact_pcd`.
    Note, however, that the solution is only a local optimum.

    Args:
        **obj_contact_pcd**: Batch of `d`-dimensional points
            of shape `(minibatch, num_points_X, d)` or a `Pointclouds` object.
        **hum_contact_pcd**: Batch of `d`-dimensional points
        ...
        **obj_contact_normals**: Optional tensor of shape `(minibatch, num_points_X, d)`
            containing the normals of the points in `obj_contact_pcd`.
        **hum_contact_normals**: Optional tensor of shape `(minibatch, num_points_Y, d)`
            containing the normals of the points in `hum_contact_pcd`.
        **min_distance**: Minimum distance to maintain between points to avoid penetration.
        **min_scale**: Minimum scale value to enforce.
        **scale_penalty**: Penalty factor for scales below `min_scale`.
        **human_mesh**: Full human mesh points of shape `(minibatch, num_points_H, d)`.
        **object_mesh**: Full object mesh points of shape `(minibatch, num_points_O, d)`.
    ...
    Returns:
        ...
        **Xt**: The point cloud `obj_contact_pcd` transformed with the final transformation
            (`R`, `T`, `s`). If `obj_contact_pcd` is a `Pointclouds` object, returns an
    ...
    obj_contact_t, num_points_X = oputil.convert_pointclouds_to_tensor(obj_contact_pcd)
    hum_contact_t, num_points_Y = oputil.convert_pointclouds_to_tensor(hum_contact_pcd)

    b, size_X, dim = obj_contact_t.shape

    if (obj_contact_t.shape[2] != hum_contact_t.shape[2]) or (
        obj_contact_t.shape[0] != hum_contact_t.shape[0]
    ):
        raise ValueError(
            "Point sets X and Y have to have the same " + "number of batches and data dimensions."
        )

    if (
        (num_points_Y < hum_contact_t.shape[1]).any()
        or (num_points_X < obj_contact_t.shape[1]).any()
    ) and (num_points_Y != num_points_X).any():
        # we have a heterogeneous input (e.g. because X/Y is
        # an instance of Pointclouds)
        mask_X = (
            torch.arange(size_X, dtype=torch.int64, device=obj_contact_t.device)[None]
            < num_points_X[:, None]
        ).type_as(obj_contact_t)
    else:
        mask_X = obj_contact_t.new_ones(b, size_X)

    # clone the initial point cloud
    obj_init = obj_contact_t.clone()
    ...
    if init_transform is not None:
        ...
        obj_contact_t = _apply_similarity_transform(obj_contact_t, R, T, s)
    else:
        # initialize the transformation with identity
        R = oputil.eyes(dim, b, device=obj_contact_t.device, dtype=obj_contact_t.dtype)
        T = obj_contact_t.new_zeros((b, dim))
        s = obj_contact_t.new_ones(b)
    ...
    obj_t_combined = (
        torch.cat([obj_contact_t, obj_contact_normals], dim=-1)
        if obj_contact_normals is not None
        else obj_contact_t
    )
    hum_t_combined = (
        torch.cat([hum_contact_t, -hum_contact_normals], dim=-1)
        if hum_contact_normals is not None
        else hum_contact_t
    )
    # the main loop over ICP iterations
    for iteration in range(max_iterations):
        nn_hum_pts = knn_points(
            obj_t_combined,
            hum_t_combined,
        ...
        nn_hum_pts, nn_hum_normals = nn_hum_pts[..., :3], -nn_hum_pts[..., 3:]
        ...
        obj_contact_t = _apply_similarity_transform(obj_init, R, T, s)
        ...
        Xt_sq_diff = ((obj_contact_t - nn_hum_pts) ** 2).sum(2)
        rmse = oputil.wmean(Xt_sq_diff[:, :, None], mask_X).sqrt()[:, 0, 0]

        Xt_normals_transformed = _apply_similarity_transform(
            nn_hum_normals, R, T=torch.zeros_like(T), s=s
        )
        cosine_similarity = torch.sum(Xt_normals_transformed * nn_hum_normals, dim=2)
        normal_rmse = 1 - cosine_similarity

        combined_rmse = rmse + normal_rmse

        # * Penalty for penetration
        # combined_rmse += 1 * penet_hum_mask.sum() / hum_verts.shape[1]

        if min_scale is not None:
            combined_rmse += scale_penalty * torch.clamp_min(s - min_scale, 0)

        # compute the relative rmse
        if prev_rmse is None:
            relative_rmse = combined_rmse.new_ones(b)
        else:
            relative_rmse = (combined_rmse - prev_rmse) / prev_rmse
    ...
    if oputil.is_pointclouds(obj_contact_pcd):
        obj_contact_t = obj_contact_pcd.update_padded(obj_contact_t)  # type: ignore

    return ICPSolution(converged, rmse, obj_contact_t, SimilarityTransform(R, T, s), t_history)
...
def corresponding_points_alignment(
    ...
    if (num_points < Xt.shape[1]).any() or (num_points < Yt.shape[1]).any():
        ...
        mask = (
            torch.arange(n, dtype=torch.int64, device=Xt.device)[None] < num_points[:, None]
        ).type_as(Xt)
    ...
    if (S.abs() <= AMBIGUOUS_ROT_SINGULAR_THR).any() and not (num_points < (dim + 1)).any():
        warnings.warn(
