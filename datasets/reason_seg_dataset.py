...
class ReasonSegDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.explanatory != -1 and image_name in self.img_to_explanation:
            # this is a work-around for working with multiview code where we need to
            # 1 class_per_sample, so it should choose 0 or 1 ([SEG] token or [SEG] token + text answer)
            if self.num_classes_per_sample == 1:
