...
from constants import AFFORD_LIST_PIAD, AFFORD_PROB_PIAD

def extract_point_file_piad(path):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_type', type=str, default='seen', help='seen or unseen')

    args = parser.parse_args()

    piad_obj_affordance_dict = {}
    split = args.split
    ...
    KERNEL_SIZE = 5
    ROOT_DIR = f'./data'
    IMG_FOLDER_PIAD = f'{ROOT_DIR}/piad_ocontact_{args.data_type}/Img/{split}'
    OUTPUT_DIR = f'{ROOT_DIR}/piad_ocontact_{args.data_type}/{args.output_dir}/{split}'

    print(IMG_FOLDER_PIAD, split, OUTPUT_DIR)
    ...
    for obj in sorted(os.listdir(IMG_FOLDER_PIAD)):
        afford_list = os.listdir(os.path.join(IMG_FOLDER_PIAD, obj))
        piad_obj_affordance_dict[obj] = afford_list

    print(piad_obj_affordance_dict)

    OBJ_LIST_PIAD = f'{ROOT_DIR}/piad_ocontact_{args.data_type}/Point_{split}.txt'
    objs_piad = open(OBJ_LIST_PIAD, 'r').read().split('\n')

    count, max_count = 0, -1
    total_objs = len(objs_piad)
    disp_img_list = []

    # random.shuffle(objs_piad)

    # run over all objects
    for obj_f in objs_piad:

        if count >= max_count and max_count > 0:
                break

        pc_cord, afford_dict, obj_name = extract_point_file_piad(obj_f)
        pc_cord = normalize_point_cloud(pc_cord)


        # for each affordance of that object
        for afford_label in piad_obj_affordance_dict[obj_name]:
            
            save_dir = os.path.join(OUTPUT_DIR, obj_name, afford_label)
            img_name = '_'.join(obj_f.split('/')[-1][:-4].split('_')[2:])
            os.makedirs(save_dir, exist_ok=True)
            try:
                print(f'\n{count}/{total_objs} Processing object: {obj_name}, affordance: {afford_label}')
                # get the affordance point cloud
                afford_pc = afford_dict[:, np.argwhere(AFFORD_LIST_PIAD == afford_label).item()]
                ...
                for save_str, camera_params in views:
                    ...
                    if os.path.exists(mask_path) and os.path.exists(sam_path):
                        print(f'----> File exists: {mask_path} and {sam_path}')
                        continue
                    print(f'saving to {mask_path} and {sam_path}')
                    ...
                    # image_color = smooth_mask(image_color, KERNEL_SIZE)
