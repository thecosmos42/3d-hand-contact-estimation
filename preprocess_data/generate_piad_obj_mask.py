...
if __name__ == '__main__':
    ...
    KERNEL_SIZE = 20
    ROOT_DIR = f'./data'
    IMG_FOLDER_PIAD = f'{ROOT_DIR}/piad_ocontact_seen/Img/{split}'
    OUTPUT_DIR = f'{ROOT_DIR}/piad_ocontact_seen/{args.output_dir}/{split}'
    ...
    OBJ_LIST_PIAD = f'{ROOT_DIR}/piad_ocontact_seen/Point_{split}.txt'
    ...
    for obj_f in objs_piad:
        ...
        if obj_name not in AFFORD_PROB_PIAD.keys():
            continue

        # for each affordance of that object
        for afford_label in piad_obj_affordance_dict[obj_name]:

            if afford_label not in list(AFFORD_PROB_PIAD[obj_name].keys()):
                continue
            ...
            try:
                afford_prob = AFFORD_PROB_PIAD[obj_name][afford_label]
                ...
                afford_pc = np.argwhere(afford_dict[:, np.argwhere(AFFORD_LIST_PIAD == afford_label).item()] > afford_prob).squeeze()
